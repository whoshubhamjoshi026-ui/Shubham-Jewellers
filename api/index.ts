...
function loadData<T>(filename: string, defaultValue: T): T {
  ...
  try {
    ...
    console.error(`[Persistence Warning] Failed to read ${filename}, using default:`, err);
  }
  return defaultValue;
}
...
function saveData<T>(filename: string, data: T): void {
  ...
  try {
    ...
    console.error(`[Persistence Warning] Failed to save ${filename}:`, err);
  }
}
...
// In-memory data structures initialized with defaults / file fallback
...
// Helper function to send OTP via Gmail SMTP
...
// Admin PIN Rate Limiting & Lockout
